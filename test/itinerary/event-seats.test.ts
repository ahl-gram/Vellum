...
// #463 part 4/4: the Ribbon room's itinerary leans the Glass on the stretch a row names, so every event has a seat on the scroll: the strip whose span holds its distance, closed at the road's end so the destination is seated too.
...
test("stripFor picks the strip whose span holds the distance, the low end closed, the road's end on the last strip", () => {
  ...
  assert.equal(stripFor(layout, input.totalCells).index, last.index, "the destination stands on the last strip");
  assert.equal(stripFor(layout, 0).index, 0);
});

test("every event of the default road seats inside its own strip's box, the departure low on the first strip and the arrival high on the last", () => {
