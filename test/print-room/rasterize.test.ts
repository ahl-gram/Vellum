...
// A browser canvas has a hard pixel ceiling the CLI never faced; over it, toBlob silently returns a smaller or null image.
...
test("fitScaleToBudget clamps a request that busts the budget (Grand 4200 x2)", () => {
  ...
  // The clamped scale sits the render EXACTLY on the budget (area * scale^2 == budget), never over it.
  assert.ok(
