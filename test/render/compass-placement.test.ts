...
function synthCtx(elev: Field, seaLevel: number): RenderCtx {
  // oceanDist mirrors the real world build: hops from the nearest land, so it is large deep inside ANY water body, which is exactly why lakes fool it.
...
// A local copy so the test states the contract without importing the geometry the implementation uses to satisfy it.
