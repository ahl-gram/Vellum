...
// Every fixture puts sea on the EAST and blows the wind WESTWARD (dir = PI points toward -x, the nautical-arrow convention), so east is upwind.
...
test("a north-blowing wind wets the north-facing side (vertical component)", () => {
  // Nothing else exercises uy, so a flipped sign would only surface as re-pinnable golden noise; sea to the NORTH, wind blowing SOUTH (dir = PI/2 points +y).
  const w = 40;
  const h = 80;
