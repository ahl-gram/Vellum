...
const MEASURED_SECONDS: Readonly<Record<E2eSuiteName, number>> = {
  ...
  "chart-drawer": 66.0, // re-measured 2026-09-19 with #634's six checks (45 total): three local single-suite runs on the shipped shape read 61.4, 61.5 and 61.7, so 66.0 is the worst case plus about 7%. It leaves lane B at 59.7% against the 0.6 this file reds at, which is 1.5s of headroom: the NEXT addition to lane B rebalances the lanes rather than raising a number. The six runs at 58.7 to 60.3 were the 44-check suite before the cold review on PR #635 added CD41, and one run at 67.9 is excluded and named: it is the same 45 checks with CD41 leaving by the Prospect page, whose plate render bought the check nothing and cost the lane 6.5s, so it goes out by the FAQ instead. Lane B's headroom is what bounds this from above, not the sweep: at 54.0 the lane sat at 58.7% of measured serial cost against the 0.6 this file reds at, which left 15.95s and a hard cap of 69.95. A fifth run that read 88.1 is excluded and named: a 30s readiness wait timed out in it, the check has since been rewritten and the failure carries a row in the flake record. CD23's 8.45s of SAY_HOLD_MS + SAY_FADE_MS is still a floor no machine can undercut, so the PRINCIPLE bounds it from below and the sweep sets the budget above. The history: 54.0 was the 39-check suite at #631, 45.7 the 23-check one at #547, 6.9 the 11-check one at #520
