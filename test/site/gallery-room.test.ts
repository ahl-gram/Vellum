...
test("GR6 the css: twelve sheets at the house depth on the deep, captions lettered in parchment, the chart room's scroll lock lifted, the grid landing, the legend centred and standing on a phone, print standing down", () => {
  ...
  const pool = kit.match(/\n([^\n]*body\.chart-room:not\(:has\(\.stage\)\) :is\(header\.chrome, \.corner:not\(\.zoomery\)\)::before[^{]*)\{([^}]*)\}/);
  assert.ok(pool, "the kit pools the cluster and the folio corners of a stage-less chart room");
  ...
  assert.match(pool![1]!, /body:has\(#map-viewport\.zoomed\) :is\(header\.chrome, \.corner:not\(\.zoomery\), \.strip\)::before/, "one rule with the zoomed rooms' pool, so the two cannot drift");
