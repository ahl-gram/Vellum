...
export async function run(ctx: SuiteContext): Promise<void> {
  ...
  // F1 and F2 are deliberately not stepped: they read the CSSOM, with nothing to wait on.
  const step = makeStep(ctx);
  await step("D1, D2", async () => {
    ...
    const d1 = await evaluate<{ arriving: boolean; dashed: boolean }>(`(()=>{const svg=document.querySelector("#map svg");const p=svg.querySelector("#layer-land path");return{arriving:svg.classList.contains("arriving"),dashed:!!(p&&p.style.strokeDasharray)};})()`);
    check("D1 fresh draw runs the arrival ceremony (svg.arriving + coast dashed mid-draw)", d1.arriving && d1.dashed, JSON.stringify(d1));
    const d2 = await evaluate<{ tform: string; dash: string; running: number }>(`(async()=>{const svg=document.querySelector("#map svg");await Promise.all(svg.getAnimations({subtree:true}).map(a=>a.finished.catch(()=>{})));const p=svg.querySelector("#layer-land path");return{tform:getComputedStyle(svg).transform,dash:p?p.style.strokeDasharray:"(no path)",running:svg.getAnimations({subtree:true}).filter(a=>a.playState==="running").length};})()`, true);
  ...
  await step("D3", async () => {
    ...
    const d3drag = await evaluate<boolean>(`document.querySelector("#map svg").classList.contains("arriving")`);
    ...
    const d3rel = await evaluate<boolean>(`document.querySelector("#map svg").classList.contains("arriving")`);
  ...
  const folio = await evaluate<{ topAuto: boolean; reducedNone: boolean }>(`(()=>{
