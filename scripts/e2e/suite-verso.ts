...
export async function run(ctx: SuiteContext): Promise<void> {
  ...
  await step("V setup", async () => {
    ...
    vTitle = await evaluate<string>(`window.__vellumRunInline({kind:"draw",seed:42,overrides:{},render:{style:"antique",widthPx:1500,legend:true}}).title`);
  });
  ...
  // @ts-expect-error the title is null when V setup never reached its read, and includes() searches for the text "null", so the clause reads false and V1b reds by name
