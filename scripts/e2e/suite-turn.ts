...
export async function run(ctx: SuiteContext): Promise<void> {
  ...
  await step("T1, T1b", async () => {
    ...
    const gPlaces = await evaluate<number>(`window.__vellumRunInline({kind:"draw",seed:42,overrides:{},render:{style:"antique",widthPx:1500,legend:true}}).manifest.places.length`);
    await armTurnWatch();
    ...
    const g1 = await evaluate<{ turned: boolean; svgCount: number; style: string | null; seed: string | null; back: number; turning: boolean; hits: number; cap: boolean }>(`(()=>{const svgs=document.querySelectorAll("#map svg");const svg=svgs[0];const back=document.querySelectorAll(".sheet-back").length;const turning=!!document.querySelector(".sheet.turning");const hits=document.querySelectorAll("#map .place-hit").length;return{turned:window.__turned,svgCount:svgs.length,style:svg?svg.getAttribute("data-vellum-style"):null,seed:svg?svg.getAttribute("data-vellum-seed"):null,back,turning,hits,cap:document.getElementById("caption").textContent.length>0};})()`);
  ...
  await step("T2", async () => {
    ...
    const g2 = await evaluate<{ turned: boolean; seed: string | null; svgCount: number }>(`(()=>{const svg=document.querySelector("#map svg");return{turned:window.__turned,seed:svg?svg.getAttribute("data-vellum-seed"):null,svgCount:document.querySelectorAll("#map svg").length};})()`);
  ...
  await step("T3, T4", async () => {
    ...
    const g3 = await evaluate<{ style: string | null; svgCount: number; back: number; turning: boolean; anims: number }>(`(()=>{const svg=document.querySelector("#map svg");const inner=document.getElementById("sheet-inner");const anims=inner&&inner.getAnimations?inner.getAnimations().length:-1;return{style:svg?svg.getAttribute("data-vellum-style"):null,svgCount:document.querySelectorAll("#map svg").length,back:document.querySelectorAll(".sheet-back").length,turning:!!document.querySelector(".sheet.turning"),anims};})()`);
  ...
  await step("T5", async () => {
    ...
    await sleep(200);
    const g5 = await evaluate<{ seed: string | null; svgCount: number; back: number; turning: boolean }>(`(()=>{const svg=document.querySelector("#map svg");return{seed:svg?svg.getAttribute("data-vellum-seed"):null,svgCount:document.querySelectorAll("#map svg").length,back:document.querySelectorAll(".sheet-back").length,turning:!!document.querySelector(".sheet.turning")};})()`);
  ...
  await step("T6b", async () => {
    await waitSettled("g6-settle");
    const g6b = await evaluate<{ seed: string | null; hits: number; svgCount: number }>(`(()=>{const svg=document.querySelector("#map svg");return{seed:svg?svg.getAttribute("data-vellum-seed"):null,hits:document.querySelectorAll("#map .place-hit").length,svgCount:document.querySelectorAll("#map svg").length};})()`);
