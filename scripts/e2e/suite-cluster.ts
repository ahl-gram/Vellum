...
type Door = { t: string; x: number; y: number; h: number; right: number; bottom: number; tappable: boolean };
type Nav = { rect: Rect; visibility: string; transform: string; position: string; transition: string; pointer: string };
type Drawer = { innerW: number; scrollW: number; inkRight: number; scrollY: number; chromeZ: string; stageInert: boolean; seedInert: boolean; shelfInert: boolean; footerInert: boolean; chromeInert: boolean; scrimHitAt400: string | null; checked: boolean; burger: Rect; burgerReachable: boolean; cluster: Rect; tagline: Rect; seed: Rect; seedOpacity: string; seedPointer: string; input: Rect; inputWidth: number; nav: Nav; scrim: { position: string; pointer: string; z: string; content: string }; doors: Door[] };
type NotHome = { page: string; scrollW: -1; checked?: undefined; burger?: undefined; doors?: undefined; inkRight?: undefined; seed?: undefined; nav?: undefined; scrollY?: undefined; seedOpacity?: undefined; stageInert?: undefined; shelfInert?: undefined; scrimHitAt400?: undefined };
...
const DRAWER_READ: Payload<Drawer | NotHome> = `(() => {
...
export async function run(ctx: SuiteContext): Promise<void> {
  ...
  const alpha = wash ? parseFloat((wash.bg.match(/\/\s*([\d.]+)\)/) || wash.bg.match(/rgba\([^)]*,\s*([\d.]+)\)/) || [])[1] ?? "1") : 0; // eslint-disable-line @typescript-eslint/no-unnecessary-condition
  check(
    ...
    !!cam && !!wash && wash.image === "none" && /blur\(/.test(wash.filter) && alpha >= 0.8 // eslint-disable-line @typescript-eslint/no-unnecessary-condition
  ...
  check(
    ...
    !!phoneCam && !!closed && closed.scrollW === 390 && clear(closed) && !closed.checked && offLeft(closed.nav) // eslint-disable-line @typescript-eslint/no-unnecessary-condition
  ...
  for (const w of [360, 320]) {
    ...
    narrows.push({ w, cam: !!cam, scrollW: s?.scrollW, inkRight: s?.inkRight, seedX: s?.seed?.x, ok: !!cam && !!s && s.scrollW === w && clear(s) }); // eslint-disable-line @typescript-eslint/no-unnecessary-condition
  }
  check(
