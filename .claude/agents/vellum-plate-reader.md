...
**`specs/settle-doctrine.md` is the tracked home for what the harness and the headless browser do**, and its "The environment" section carries the window-width clamp, focus emulation, what the server actually serves and how a visual claim is controlled. Read it before any narrow-width, focus-state or screenshot check, and when you learn a new environment fact, record it there and not only here. The traps below are this agent's own working set and several of them are the same facts said twice.
...
- **`Page.captureScreenshot` with `captureBeyondViewport: true` drops a chart room's fixed furniture from the frame.** Measured 2026-09-03 on /specimen/ at 1280x800: the chart folio (bottom left) and the legend row vanished while the right-anchored corners and the slip stayed, AE 505 between the two modes at the same instant, computed opacity 1 on every piece. A crop of the empty deep then "measures" 1.06:1 and reports invisible furniture (this agent did exactly that on #465). Use beyond-viewport capture only for a SCROLLING full-page shot; on a chart room, or for any clip inside the viewport, leave it off (the harness's `shoot(file, clip)` does so for an explicit clip).
...
`out/324-audit-shoot.mjs` is a working template for a multi-page shot driver built on it. Others worth cribbing: `out/shoot-shell.mjs`, `out/220-shoot-live.mjs`, `out/probe-runninghead.mjs`. They predate the harness's move to TypeScript and import it as `harness.mjs`, so a copy changes that one specifier to `harness.ts`.
