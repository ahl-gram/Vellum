...
7. **Ask what the plan drags with it.** A new page, sheet, suite, room, CSS surface or support module joins rosters that fail silently. A behavior change that contradicts a ratified line in any spec under `specs/` or in the footguns skill has to edit that line. A plan that mentions neither has probably seen neither.
8. **Your own probes are subject to the measurement traps**, and none of them throws, they just return plausible wrong numbers: the seed comes FIRST in `defaultRecipe(seed, overrides)`; `PlaceMark.nx/ny` are rendered-chart space and cannot sample terrain, use `world.settlements[i].x/y`; `world.elev` is a `Field` read with `.at(x, y)` while `world.oceanDist` is a bare `Float64Array` indexed `y * W + x`. `specs/engine-invariants.md` ("Measuring a world in a script") is the authority on these and this is a deliberate copy of it, carried the way `vellum-pr-skeptic` carries one: whether a subagent reads either file is a fact about the harness that no command in this repo settles, and a probe that measured the wrong thing is a worse outcome than a paragraph that has to be kept in step.
...
- **One language, one pipeline.** New code is TypeScript under `src/`, and no JavaScript file is tracked outside `design/`: `npm run lint` refuses one (Issue #653 ruling D). A plan that adds one plans a red lint, or an exemption, which is a rule-level block in `eslint.config.ts` named for the ruling that admits it.
