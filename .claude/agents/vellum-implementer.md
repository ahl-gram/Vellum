...
- Open the PR with `gh pr create --body-file`, in the shape of `.github/PULL_REQUEST_TEMPLATE.md`: every one of its `## ` sections present, since the hook refuses a body that skips one, AND the closing-reference line that template asks for, or "No issue:" with the reason where there is none. Title starts with `#N`. Then `vellum-pr-skeptic`, dispatched COLD with the PR number and nothing else. Fix, re-prove, at most three rounds; residue named in the body.
