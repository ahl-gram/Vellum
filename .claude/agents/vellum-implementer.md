...
- Open the PR with `gh pr create --body-file`, in the shape of `.github/PULL_REQUEST_TEMPLATE.md`: every one of its `## ` sections present, since the hook refuses a body that skips one, AND a closing reference, `Closes #N` for the issue this PR finishes or "No issue:" with the reason where there is none, in the place the template gives it. Title starts with `#N`. Then `vellum-pr-skeptic`, dispatched COLD with the PR number and nothing else. Fix, re-prove, at most three rounds; residue named in the body.
