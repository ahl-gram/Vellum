...
- The scratchpad is SHARED with any parallel implementer. Prefix every scratch file with the issue number (`N-plan.md`, `N-pr-body.md`, `N-probe.ts`) and read a file back before relying on it.
...
- Open the PR with `gh pr create --body-file`, in the shape of `.github/PULL_REQUEST_TEMPLATE.md`: every one of its `## ` sections present, since the hook refuses a body that skips one, AND a closing reference, whose forms the template's own comment enumerates beside the line; write the one that fits and do not carry a second copy of the list here. Title starts with `#N`. Then `vellum-pr-skeptic`, dispatched COLD with the PR number and nothing else. Fix, re-prove, at most three rounds; a finding you do not fix is filed or added to `errata/` as a row in the same diff (workflow step 15), never left as prose in the body.
