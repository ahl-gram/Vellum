...
| 2026-08-16 | BR8 | not retained | not retained, #409 | `AbortError: Transition was skipped` counted as a console error | settled, #409: a cross-document view transition's expected cancellation. That payload is one member of a FAMILY, not the wording: the browser composes the message as one of three openings plus one of eighteen reasons, so #613 (2026-09-14) found a second opening counting as an app error and moved the whole family behind one drop in `scripts/e2e/console-support.ts`, holding out the reasons that name our own stylesheet |
...
| 2026-09-14 | lane A launch | 34883972991 | `e21af5a`, `fix/613-e2e-console-filter-transition-wording` | `no devtools page target (browser exited code=null signal=SIGKILL)` after three launch attempts within 0.3s at 18:58:52 to 18:58:53 UTC, no check run, browser output `(none captured)`; lane B `ALL PASS (303/303)` | open, no issue. Second lane A launch SIGKILL today, same shape as the row above: three attempts in the same second, no check run, lane B passed 303/303 on the sha. Recorded before the push that is its only re-run |
| 2026-09-19 | CD36 | local single-suite run, not CI | `4d3655a`, `fix/634-table-two-homes` | `FAIL CD36 never reached its assertion`, with the message `the Explorer never booted` and NOTHING else: the arrival was a throwing readiness wait with no payload, which is why this row cannot say where the browser was standing. The two parts are quoted separately because that is how `makeStep` prints them, the check name and the message, and a single reconstructed line in a record whose whole job is the pasted one is worse than a gap (the cold review on PR #635 caught this row inventing a separator). The run's own wall clock does carry one fact: 88.1s against 58.7s on the next run, so the 30s the wait burned is the whole difference and every other check passed in both | settled, PR for Issue #634, and recorded honestly: the row went in AFTER the re-run rather than before it, against Gate 2 item 10. The check was a brand new one, red on its first run; the arrival was rewritten from a throwing wait into a measurement poll whose last read the check asserts on (settle-doctrine clause 4), so a next occurrence names the page the press left the browser on, and the press now waits for the Portfolio to finish drafting before it fires, which removes the worker contention that is the only cause consistent with the timing. Cause NOT established |
| 2026-09-23 | CD25, CD26, CD30 (the step; CD25 and CD26 had passed, the second prospect never filed) | 35918976248 | `5f27f48`, `chore/653-e2e-ts-room` | `CD25, CD26, CD30 never reached its assertion: settle timeout chart-drawer-prospect-second: {"hits":26,"shown":false,"name":"Weki","press":{"text":"Lay the prospect on the table","dim":false,"idx":"2","box":{"x":0,"y":0,"w":0,"h":0},"hit":"no-box","disabled":false},"link":{"hit":"no-box","inActs":true},"actsRow":2,"pressInActs":true,"cuttings":1,"prospects":1,"titles":["The Prospect of Paukilua"],"subs":["a prospect, antique, 1059"],"imgs":1,"decoded":[true],"frames":0,"hashTable":"k-p.seed-42.style-antique.i-1.year-1059","seedBox":"7","scrollW":1280,"innerW":1280}`: the second place card is not shown and its press has no box, so the second press never lands | open. The head's runtime tokens are identical to `de4dd82`, whose CI run 35916661918 passed this step, and chart-drawer is not a suite that PR changes; re-run once at the next head |
| 2026-09-26 | lane A launch | 36239544995, attempt 1 | `9892939`, `chore/654-row3-index-switch` | `HARNESS ERROR: Error: no devtools page target (browser exited code=null signal=SIGKILL)` after three launch attempts at 11:41:22 UTC, no check run, browser output two dbus `Failed to connect to the bus` lines; lane B and check & test passed on the sha | open, no issue. The same shape as the two 2026-09-14 rows. Recorded late: attempt 2 was run from the PR #690 lane without a row first, and passed |
| 2026-09-26 | `rasterizeSvg produces a real PNG` (`test/cli/raster.test.ts`) | local full `npm test`, not CI (the test skips under CI) | `b1b6917`, `chore/654-row4-assertions` | `Error: Command failed: /Applications/Brave Browser.app/Contents/MacOS/Brave Browser --headless --disable-gpu --hide-scrollbars --screenshot=out/test-tmp/tiny.png --window-size=120,90 ...`, repeated `CVDisplayLinkCreateWithCGDisplay failed. CVReturn: -6670`, `287 bytes written to file out/test-tmp/tiny.png`, then `Teardown watchdog expired; recording dump and terminating.` (exit code 2) | open, no issue. The machine was also running a 100-seed byte sweep and a guard-prover's unit runs; the test passed alone and the full suite then passed alone at the same sha. Recorded late: the re-run came before this row |
