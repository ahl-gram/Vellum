...
- **Injects a gate once per session**, reading the text from `SKILL.md` so the skill stays the single
  source: Gate 1 on `test/**/*.test.ts`; Gate 2 on `scripts/**/*.mjs` and `out/**/*.mjs`, and on `.ts` under `scripts/e2e/`, `out/` and `scripts/e2e-*` (the port-proof tool and the two runners; never all of `scripts/**/*.ts`, since the first matching route wins and Gate 6 owns the render scripts); Gate 3 on
