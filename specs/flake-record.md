...
| 2026-09-14 | lane A, at browser launch (`HARNESS ERROR`, exit 2) | 34823116661 | `aff6082`, `chore/588-site-architecture` | `no devtools page target (browser exited code=null signal=SIGKILL)` after three launch attempts, 20.7s in, no check run; summary `LANE A FAILED (303/303 checks; A HARNESS ERROR (exit 2) 20.7s, B ok 565.5s)` | open, no issue. The first `HARNESS ERROR` row here, and the first row that is not a check at all: both lanes lost their browser to SIGKILL in the same second as they launched together, lane B recovered on its third attempt and passed 303/303 on that sha, and the diff is two markdown files, which no suite reads. Recorded before the branch was pushed again, which is the only re-run it got |
