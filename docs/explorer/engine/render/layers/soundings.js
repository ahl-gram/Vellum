 ...
 */
export function soundingsLayer(ctx, cartouche, compass) {
    ...
    const clear = (px, py) => avoid.every((b) => !boxesOverlap(b, { x: px - 8, y: py - 8, w: 16, h: 16 }, 8));
    const spots = [];
    const rocks = [];
    for (let gy = 2; gy < h - 2; gy += 2) {
        for (let gx = 2; gx < w - 2; gx += 2) {
            ...
            if (e > sea)
                continue;
            const dist = world.oceanDist[i];
            const px = proj.px(gx + srng.range(-0.6, 0.6));
            const py = proj.py(gy + srng.range(-0.6, 0.6));
            if (px < proj.margin + 14 || px > proj.widthPx - proj.margin - 14)
                continue;
            if (py < proj.margin + 14 || py > proj.heightPx - proj.margin - 14)
                continue;
            if (!clear(px, py))
                continue;
            const depth = (sea - e) / below;
            if (dist >= 2)
                spots.push({ x: px, y: py, depth, dist });
            else if (dist >= 1 && depth < 0.12)
                rocks.push({ x: px, y: py, depth, dist });
        }
    }
