...
function buildRows(ctx) {
    ...
    if (style.name === "nautical") {
        ...
        if (style.contourStroke)
            rows.push({ icon: { kind: "contour" }, label: "Contour line" });
    }
    if (world.rivers.length > 0)
        rows.push({ icon: { kind: "river" }, label: "River" });
    const roadRanks = new Set(world.roads.map((r) => r.rank));
    if (roadRanks.has("trunk"))
...
export function planLegend(ctx, reserved) {
    ...
    for (const c of corners) {
        const box = { x: c.x, y: c.y, w: bw, h: bh };
...
function iconNode(icon, cx, cy, ctx) {
    const { style } = ctx;
    ...
    switch (icon.kind) {
        ...
        case "realm":
            return el("rect", {
                ...
                fill: style.realmTints[0], "fill-opacity": 0.5,
        ...
        case "hypso": {
            ...
            return el("g", {}, stops.map((s, i) => el("rect", {
