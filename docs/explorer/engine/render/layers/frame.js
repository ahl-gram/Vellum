...
export function frameLayer(ctx) {
    ...
    for (let x = m + step; x < w - m - 1; x += step) {
        ticks.push(el("line", { x1: x, y1: m, x2: x, y2: m - tickLen, stroke: style.ink, "stroke-width": 0.8 }), el("line", { x1: x, y1: h - m, x2: x, y2: h - m + tickLen, stroke: style.ink, "stroke-width": 0.8 }));
    }
    for (let y = m + step; y < h - m - 1; y += step) {
        ticks.push(el("line", { x1: m, y1: y, x2: m - tickLen, y2: y, stroke: style.ink, "stroke-width": 0.8 }), el("line", { x1: w - m, y1: y, x2: w - m + tickLen, y2: y, stroke: style.ink, "stroke-width": 0.8 }));
    }
