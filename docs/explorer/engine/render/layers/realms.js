...
import { chaikinSmooth, marchingSquares } from "../../terrain/contours.js";
import { el, pathFrom } from "../svg.js";
function boxBlur(f, passes) {
    let cur = f;
...
export function realmTintsLayer(ctx) {
    const { world, proj, style } = ctx;
    const { labels, seats } = world.realms;
    if (!style.politicalTints || seats.length <= 1)
        return null;
    ...
    for (let realm = 0; realm < seats.length; realm++) {
        ...
        nodes.push(el("path", {
            d,
            fill: style.realmTints[realm % style.realmTints.length],
...
function chainBorderSegments(segs) {
        ...
        for (const end of [1, 0]) {
            for (;;) {
                ...
                if (nextIdx === -1)
                    break;
                used[nextIdx] = 1;
                const t = segs[nextIdx];
                const startsAtTip = key(t[0], t[1]) === key(tip[0], tip[1]);
                const far = startsAtTip ? [t[2], t[3]] : [t[0], t[1]];
                if (end === 1)
                    chain.push(far);
                else
                    chain.unshift(far);
...
export function realmBordersLayer(ctx) {
    const { world, proj, style } = ctx;
    const { labels, seats } = world.realms;
    if (seats.length <= 1)
        return null;
    ...
    for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
            ...
            if (a < 0)
                continue;
            if (x + 1 < w) {
                const b = labels[x + 1 + y * w];
                if (b >= 0 && b !== a) {
                    segs.push([x + 0.5, y - 0.5, x + 0.5, y + 0.5]);
                }
            }
            if (y + 1 < h) {
                const b = labels[x + (y + 1) * w];
                if (b >= 0 && b !== a) {
                    segs.push([x - 0.5, y + 0.5, x + 0.5, y + 0.5]);
                }
            }
    ...
    if (segs.length === 0)
        return null;
    const chains = chainBorderSegments(segs).map((chain) => chaikinSmooth(chain, false, 2).map(([x, y]) => [proj.px(x), proj.py(y)]));
    return el("g", { id: "layer-realm-borders" }, chains.map((chain) => el("path", {
