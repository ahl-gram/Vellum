...
export function seaDecorLayer(ctx, cartouche, compass) {
    const { style, world, proj, rng } = ctx;
    if (!style.seaDecorations)
        return null;
    ...
    if (compass)
        avoid.push({ x: compass.cx, y: compass.cy, r: compass.r * 2.2 });
    const clearOf = (px, py, extra = 0) => avoid.every((a) => Math.hypot(px - a.x, py - a.y) > a.r + extra);
    const edgeClear = (px, py, need) => px - proj.margin > need &&
        py - proj.margin > need &&
    ...
    const deepSpots = open
        ...
        .sort((a, b) => b.d - a.d);
    const claimSpot = (o, half) => o && ctx.labels.tryClaim({ x: o.x - half, y: o.y - half, w: half * 2, h: half * 2 }, 6)
        ? o
        : undefined;
    const serpentAt = claimSpot(deepSpots.find((o) => clearOf(o.x, o.y, 90 * k)), 60 * k);
    if (serpentAt) {
        const s = 1.5 * k;
        const { x, y } = serpentAt;
        const stroke = {
        ...
        nodes.push(el("g", { id: "sea-serpent", opacity: 0.85 }, [
            // tail fin
            el("path", {
    ...
    const shipAt = claimSpot(deepSpots.find((o) => !serpentAt ||
        Math.hypot(o.x - serpentAt.x, o.y - serpentAt.y) > 260 * k), 45 * k);
    if (shipAt) {
        const s = 1.25 * k;
        const { x, y } = shipAt;
        const stroke = {
        ...
        nodes.push(el("g", { id: "sea-ship", opacity: 0.85 }, [
            el("path", {
