import { chaikinSmooth, closedIsoRings, marchingSquares, } from "../../terrain/contours.js";
import { el, pathFrom } from "../svg.js";
export function landLayer(ctx) {
...
export function hypsometricLayer(ctx) {
    ...
    for (const stop of style.hypsometric) {
        if (stop.t === 0)
            continue; // base painted by the land layer
...
export function contoursLayer(ctx) {
    const { style, world, proj, elevSpan } = ctx;
    ...
    for (let i = 1; i <= levels; i++) {
        ...
        if (contours.length === 0)
            continue;
        const d = contours
            .map((c) => pathFrom(chaikinSmooth(c.points, c.closed, 2).map(([x, y]) => [proj.px(x), proj.py(y)]), c.closed))
            .join("");
