...
export function featureLabelsLayer(ctx) {
    ...
    for (let gy = 3; gy < h - 3; gy += 2) {
        for (let gx = 3; gx < w - 3; gx += 2) {
            ...
            if (d >= 5)
                deep.push({ px: proj.px(gx), py: proj.py(gy), d });
        }
    }
    ...
    for (const fsBase of [26, 21, 17]) {
        ...
        for (const cand of deep) {
            ...
            if (box.x < proj.margin + 8 || box.x + box.w > proj.widthPx - proj.margin - 8)
                continue;
            if (box.y < proj.margin + 8 || box.y + box.h > proj.heightPx - proj.margin - 8)
                continue;
            if (!labels.tryClaim(box, 8))
                continue;
            nodes.push(el("text", {
    ...
    for (const lake of world.names.lakes) {
        ...
        const fs = 11.5 * k;
        const placedY = [ly, ly - 15 * k, ly + 15 * k].find((cy) => labels.tryClaim(textBox(lx, cy, lake.name, fs, "middle"), 3));
        if (placedY === undefined)
            continue;
        nodes.push(el("text", {
    ...
    if (world.names.range) {
        ...
        if (blob.length >= 10) {
            ...
            const fs = 14.5 * k;
            const placedY = offsetCandidates(c.y, k).find((cy) => labels.tryClaim(spacedTextBox(c.x, cy, world.names.range, fs, 3 * k), 4));
            if (placedY !== undefined) {
                nodes.push(el("text", {
                    x: c.x, y: placedY, "text-anchor": "middle",
    ...
    if (world.names.forest) {
        const blob = largestBlob(w, h, (i) => FOREST_BIOMES.has(world.biomes[i]));
        if (blob.length >= 25) {
            const pts = blob.map((i) => ({
                x: proj.px(i % w),
                y: proj.py((i / w) | 0),
            ...
            const c = centroidOf(pts);
            const fs = 12.5 * k;
            const placedY = offsetCandidates(c.y, k).find((cy) => labels.tryClaim(textBox(c.x, cy, world.names.forest, fs, "middle"), 4));
            if (placedY !== undefined) {
                nodes.push(el("text", {
                    x: c.x, y: placedY, "text-anchor": "middle",
                    "font-family": style.fontFamily,
