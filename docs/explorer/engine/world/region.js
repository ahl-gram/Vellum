...
import { classifyBiomes } from "../climate/biomes.js";
 ...
 */
export function generateRegionWorld(world, spec) {
    const { recipe } = world;
    ...
    for (const s of world.settlements) {
        ...
        if (u < window.u0 + du * inset || u > window.u1 - du * inset ||
        ...
        if (elev.data[gx + gy * gridW] <= seaLevel) {
            let snapped = false;
            for (const [dx, dy] of NEIGHBORS_8) {
                const nx = clamp(gx + dx, 0, gridW - 1);
                const ny = clamp(gy + dy, 0, gridH - 1);
                if (elev.data[nx + ny * gridW] > seaLevel) {
                    gx = nx;
                    gy = ny;
                    snapped = true;
                    break;
...
export function windowAround(world, s, size) {
