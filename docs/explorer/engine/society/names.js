...
export function createNamer(rng, culture) {
    const used = new Set();
    let overflow = 0;
    const rawBase = () => {
        ...
        for (const ch of pattern) {
            if (ch === "O")
                s += rng.pick(culture.onsets);
            else if (ch === "N")
                s += rng.pick(culture.nuclei);
            else
                s += rng.pick(culture.codas);
        }
        return s;
    };
    const uniqueBase = (suffix) => {
        for (let attempt = 0; attempt < 30; attempt++) {
            let s = rawBase();
            if (s.length < 3 || s.length > 9 || s.length + suffix.length > 13)
                continue;
            if (/[aeiou]{3}/.test(s) || /(.)\1\1/.test(s))
                continue;
            if (suffix) {
                if (s.endsWith(suffix[0]))
                    s = s.slice(0, -1);
                else if (/[aeiou]$/.test(s) && /^[aeiou]/.test(suffix))
                    s = s.slice(0, -1);
                if (s.length < 3)
                    continue;
            }
...
export function makeMapTitle(rng, culture, mapType, baseOverride) {
    ...
    const subtitle = `Being a true & faithful chart of these waters, as surveyed by ` +
