 ...
 */
export function buildRoads(elev, seaLevel, riverCells, settlements) {
    const { w, h, data } = elev;
    const n = w * h;
    ...
    const terrainCost = (i) => 1 +
        slope.data[i] * SLOPE_PENALTY +
        (riverCells[i] === 1 ? RIVER_CROSSING : 0);
    const roads = [];
    const connect = (sx, sy, rank, budget) => {
        const start = sx + sy * w;
        if (network[start])
            return;
        ...
        if (found === -1)
            return; // unreachable (another island) — no road
        const points = [];
        let cur = found;
        while (cur !== -1) {
