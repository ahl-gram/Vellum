...
engine/CLI/scripts natively). **No JavaScript file is tracked anywhere** (`.js`, `.mjs`, `.cjs`,
`.jsx`), and `npm run lint` is what enforces it: the blocks named for Issue #653's ruling D in
`eslint.config.ts` refuse every such file the `.gitignore` does not ignore. `public/` is static assets
only (goldens, fonts, CSS, the icons). The one exemption is `design/`, an archive whose round tools
keep the JavaScript they ran as; another exemption is a rule-level block in that config, named for
the ruling that admits it.
