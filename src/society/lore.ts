...
export function createLoreWriter(world: World, rng: Rng): LoreWriter {
  const goods = (GOODS[world.culture.id] ?? GOODS["thalassic"]) as readonly string[];
  ...
  const freshPick = (list: readonly string[]): string => {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      if (!seen.includes(candidate)) {
        seen.push(candidate);
        if (seen.length > Math.floor(list.length / 2)) seen.shift();
        return candidate;
      }
    }
  ...
  const fill = (template: string): string =>
    template.replace("%", rng.pick(goods));
