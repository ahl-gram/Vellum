// The travel matrix as a job, called by ./worker.ts and mirrored by runInline in ./worker-client.ts, so the itinerary cannot depend on which thread computed it.
