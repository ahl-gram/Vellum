...
import { DEFAULT_PACE, anchorFor, storyElapsed, type Pace } from "./pace.ts";
...
interface AgesSession {
  ...
  playing: boolean;
  rafId: number;
  /** The wall-clock anchor of the running sweep (pace.ts); 0 at rest. */
  begin: number;
  annals: AnnalRow[];
...
export function createAges(deps: AgesDeps) {
  ...
  function play(): void {
    ...
    ages.begin = anchorFor(performance.now(), elapsed0, pace);
    ages.playing = true;
    setPlayLabel(true);
    const tick = (now: number) => {
      ...
      const elapsed = Math.max(storyElapsed(now, ages.begin, pace), elapsed0);
      if (elapsed >= totalMs) {
  ...
  /** The pace for this Play and every later one; a change mid-sweep keeps the story where it stands. */
  function setPace(k: Pace): void {
    if (ages && ages.playing) {
      const now = performance.now();
      ages.begin = anchorFor(now, storyElapsed(now, ages.begin, pace), k);
    }
    pace = k;
  }
