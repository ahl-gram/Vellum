...
function geom(cx: number, cy: number, size: number): Geom {
  const w = size;
  const h = size * 1.18;
  const half = w / 2;
  return {
...
function shieldPath(g: Geom): string {
...
function fieldNodes(arms: Arms, g: Geom, pal: ArmsPalette): SvgNode[] {
  const base = el("rect", {
...
function chargeNodes(arms: Arms, g: Geom, pal: ArmsPalette): SvgNode[] {
...
export function heraldryLayer(
  ...
  let placements: ReadonlyArray<RealmAnchor> = anchors;
